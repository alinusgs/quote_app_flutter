...
const List<String> kQuotes = [
  ...
  "Верьте в себя и все, что вы делаете.",
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    String? savedQuote = prefs.getString('last_quote');
    
    if (savedQuote == null) {
      await prefs.setString('last_quote', kQuotes[0]);
      savedQuote = kQuotes[0];
    }
  ...
  // ИСПРАВЛЕННЫЙ МЕТОД ГЕНЕРАЦИИ 
  void _generateQuote() async {
    final prefs = await SharedPreferences.getInstance();
    String nextQuote;
    do {
      nextQuote = kQuotes[_random.nextInt(kQuotes.length)];
            } while (nextQuote == _currentQuote);

    await prefs.setString('last_quote', nextQuote);
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(_currentQuote, textAlign: TextAlign.center, style: const TextStyle(fontSize: 24)),
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                ...
                ElevatedButton(onPressed: _generateQuote, child: const Text('Другая цитата')),
              ],
            )
          ],
        ),
